...
- нейроотличные пользователи (РАС, СДВГ, ПРЛ, НРЛ и др.) — люди, которым регулярно нужна помощь с процессингом эмоций, декодированием социальных ситуаций, формулировкой ответов и структурированием мыслей. Бот не является медицинским инструментом, но закрывает повседневные задачи, с которыми нейроотличные люди сталкиваются чаще и острее.
- мужчины, которым сложно процессить эмоции — не могут назвать что чувствуют, социальная норма «будь сильным», стыд перед психологом, некому рассказать. Приходят через утилитарный вход («помоги ответить», «разбери ситуацию»), но получают эмоциональную ясность незаметно. Ключевые персонажи: Ян (структура вместо хаоса), Макс (снижает барьер входа через юмор и прямоту), Наташа (называет чувства, которые мужчина не может назвать сам). Бот — не терапия и не «поговори о чувствах», а практичный инструмент, через который эмоциональный процессинг происходит как побочный эффект.
...
- Как пользователь, я запускаю `📎 Собрать переписку`, отправляю несколько фрагментов и нажимаю `Готово`.
...
- Обычные входы `🚀 Спросить всех`, `📝 Напиши за меня`, `💬 Помоги ответить` принимают один следующий input и сразу запускают генерацию.
- Несколько фрагментов собираются только после явного входа `📎 Собрать переписку`.
- Pending режимы:
...
### 7.3 Inputs
- Текст и пересланный текст обрабатываются как обычный input.
- Voice до 10 минут транскрибируется и дальше идёт в тот же сценарий.
- Screenshot/картинка анализируется vision-моделью и дальше идёт в тот же сценарий.
